...
def set_supply_voltage_setpoint(housekeeper, voltage):
    ...
    # Check LE16 max value
    # TODO - move to pyedbglib
    if setpoint_mv.bit_length() > 16:
...
def showdata(data, address=0, page_size=None, line_wrap=16, phantom_bytes=0):
    """
    Show (print) the data
    ...
    """
    # If start address matches phantom byte location it should just be moved to the first location representing actual
    # data
    word_index = address % (1 + phantom_bytes)
    if word_index:
    ...
    for data_index, value in enumerate(data, div+1):
        ...
        if page_size is not None:
            if (print_index+(rows*line_wrap)) % page_size == 0 and data_index != len(data)+div:
                print("")
                wrap = True
        if print_index % line_wrap == 0 and data_index != len(data)+div or wrap:
            print("")
            print("0x{0:06X}: ".format(address-div + print_index), end='')
            wrap = False

    # Figure out how many extra empty data positions to print
    extra = line_wrap - print_index % line_wrap
...
def pad_to_size(memory_block, chunk_size, pad_value):
    ...
    """
    # Deep copy of input array
    memory_block = copy.deepcopy(memory_block)
...
def verify_from_bin(bin_filename, backend, offset=0, memory_name=MemoryNames.FLASH):
    """
    ...
    verify_status = backend.verify_memory(bin_data, memory_name, offset)
