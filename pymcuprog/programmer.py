...
class Programmer:
    ...
    def setup_device(self, interface=None, packpath=None, clk=None):
        ...
        :type packpath: str
        :param clk: Clock frequency or baud rate to use
        :type clk: int
    ...
    def verify_memory(self, data, memory_name, offset=0):
        ...
        try:
            ...
            self.logger.error("Verify failed for %s memory:", memory_name)
            self.logger.error("%s (is the memory section erased?)", str(error))
            return False
        return True
    ...
    def read_memory(self, memory_name, offset, numbytes=0):
        ...
        :type numbytes: int
        :returns: List of namedtuples with two fields: data and memory_info.  data contains a byte array
