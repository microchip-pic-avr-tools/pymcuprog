...
class TestPymcuprogCLI(unittest.TestCase):
    ...
    def delayed_memory_read_stub(self, memory_name, start, numbytes):
        ...
        # Add sleep to make timing testable
        time.sleep(1)

        # Forward call to make sense
        return self.updi_memory_read_stub(memory_name, start, numbytes)
    ...
    def test_timer_option(self, mock_read_tool_info, mock_housekeepingprotocol, mock_avr8protocol):
        ...
        # The delayed_memory_read_stub puts in a 1s delay per memory access call
        # There are 4 calls on session start + ping: SIB, CS, SRAM x 2
        # This means that the time took should start with "4."" (leaving decimals to overheads)
        self.assertTrue("Operation took 4." in self.mock_stdout.getvalue(), msg=f"Actual output {self.mock_stdout.getvalue()}")
    ...
    @patch('pymcuprog.backend.housekeepingprotocol')
    @patch('pymcuprog.backend.read_tool_info')
    def test_write_out_of_range_mega4809_causes_error(self,
                                                      mock_read_tool_info,
        ...
        # Error should return - no exception
        status = pymcuprog.pymcuprog(arguments)
        self.assertEqual(status, STATUS_FAILURE)
