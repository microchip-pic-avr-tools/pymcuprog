...
class SamD2xTarget(SamTarget):
    ...
    # Lock region size actually depends on flash size
    # TODO: abstract using device model if more SAMs are supported
    FLASH_LOCK_REGIONS = 16
    FLASH_LOCK_REGION_SIZE = (0x40000 // FLASH_LOCK_REGIONS)
    ...
    def connect(self, clk_hz):
        ...
        # LED 1 to on
        # TODO - fix after FW5G-1121
        #self.debugger.dap_led(1,1)
    ...
    def wait_flash_ready(self, timeout_ms=1000):
        ...
        :param timeout_ms:
        :return: boolean True if flash is ready; False if timeout waiting
        """
        while timeout_ms > 0:
            # Done?
    ...
    def unlock_all_regions(self):
        """
        Unlock all regions of flash.
        This only issues the NVM UR command, and has no effect on for example BOOTPROT locks.
        """
        address = 0
        for a in range(self.FLASH_LOCK_REGIONS):
            self.logger.info("Unlock region %d at address 0x%06X", a, address)
            # Write address
            self.set_nvmctrl_address(address)

            # Apply "Unlock Region" command
            self.nvm_command(self.CMD_UR)
            address += self.FLASH_LOCK_REGION_SIZE
    ...
    def pre_flash_write(self):
        """
        Prepare flash for write (done once)
        """
        # Wait for flash to be ready
        self.wait_flash_ready()
        # Disable manual commit - in this mode each flash page is auto-committed once filled
        self.debugger.write_word(self.NVM_CTRL_ADDRESS + self.NVM_CTRL_CTRLB_OFFSET, (0 << self.NVM_CTRLB_MANW_BIT))

    def post_flash_write(self):
        """
